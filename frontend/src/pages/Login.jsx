			...
			<div className="hidden lg:flex relative bg-surface items-center justify-center p-12 overflow-hidden">
				...
				<div className="relative z-10 max-w-lg space-y-8">
					...
					<h1 className="text-5xl font-display font-bold text-white">
						Continue your <br />
						<span className="text-transparent bg-clip-text bg-gradient-brand">Learning Journey.</span>
					</h1>

					<p className="text-ink-400 text-lg">
						Access your personalized dashboard, track progress, and unlock new skills with our premium courses.
					</p>
					...
					<Card className="p-0 bg-transparent border-0 shadow-none">
						<form onSubmit={submit} className="space-y-6">
								...
								>
									...
									<div className="grid grid-cols-2 gap-2 p-1 bg-surfaceHighlight rounded-xl mb-6">
										<button
											...
											className={`flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${mode === "user"
										...
										<button
											...
											className={`flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${mode === "admin"
													? "bg-brand-500 text-white shadow-lg"
													: "text-ink-400 hover:text-white"
												}`}
										>
									...
									<div className="space-y-4">
										<div className="relative">
											<Mail className="absolute left-3 top-3.5 text-ink-400" size={20} />
											<Input
												type="email"
										...
										<div className="space-y-1">
											<div className="relative">
												<Lock className="absolute left-3 top-3.5 text-ink-400" size={20} />
												<Input
													type="password"
								...
								>
									<div className="bg-brand-500/10 border border-brand-500/20 rounded-xl p-4 text-center">
										<p className="text-sm text-ink-200">
											...
											<span className="font-bold text-white">{emailForOtp || email}</span>
										</p>
									</div>

									<div className="relative">
										<Shield className="absolute left-3 top-3.5 text-ink-400" size={20} />
										<Input
											value={otp}
							...
							<Button
								...
								isLoading={loading}
							>
								{phase === "credentials" ? "Sign In" : "Verify & Login"} <ArrowRight size={18} className="ml-2" />
							</Button>
						</form>
					</Card>
				</div>
